...
var noReconnectCodes = map[int]bool{
	4012: true, 4013: true, 4014: true, 4018: true, 4019: true, 4021: true,
}
...
func New(opts map[string]any) (core.Platform, error) {
	...
	if appKey == "" || appSecret == "" {
		return nil, fmt.Errorf("yuanbao: app_key and app_secret are required")
	}
	...
	case head.cmdType == cmdTypePush:
		if head.needAck {
			if ws := p.getWS(); ws != nil {
				ws.WriteMessage(websocket.BinaryMessage, encodePushAck(head))
			}
		}
	...
	for _, item := range items {
		...
		if content, ok := item["msg_content"].(map[string]interface{}); ok {
			...
			case map[string]interface{}:
				elem.msgContent = v
			case string:
				json.Unmarshal([]byte(v), &elem.msgContent)
			}
		}
